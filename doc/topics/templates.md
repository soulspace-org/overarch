...
Overarch supports forward engineering vwith protected areas for manually written
content in generated artifacts

With templates you can customize and extend all aspects of the generation of
artifacts in Overarch without changing the Overarch codebase itself.

## Comb Templates
Overarch Templates use the syntax of the [Comb](https://github.com/weavejester/comb)
template engine, but implements it's own engine on top of the Small Clojure Interpreter (SCI).

Comb templates use embedded Clojure code as the control language, but by using
SCI, the execution is sandboxed.
...
### Clojure Namespaces
The template engine supports requiring Clojure namespaces from the templates.
This allows building a library of reuseable functions to be used across the
different templates. Overarch provides such namespaces in the `templates/lib`
folder.
...
`m` alias, e.g. `m/resolve-element`. It also provides functions to query and
navigate the views under the `v` alias.
