(ns org.soulspace.overarch.domain.views.domain-view
...
(defn render-model-node?
  ...
  (let [p (model/parent model e)]
    (contains? el/domain-view-element-types (:el e))))
...
(defn render-model-relation?
  ...
  (let [from (model/model-element model (:from e))
    ...
    (and (contains? el/domain-view-element-types (:el e))
         (render-model-node? model view from)
         (render-model-node? model view to))))

(defmethod view/render-model-element? :domain-view
  [model view e]
  (if (el/model-relation? e)
    (render-model-relation? model view e)
    (render-model-node? model view e)))

(defmethod view/include-content? :domain-view
  [model view e]
  (and (contains? el/domain-view-element-types (:el e))
       (el/boundary? e)))

(defmethod view/element-to-render :domain-view
  [model view e]
  e)
