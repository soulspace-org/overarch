...
(deftest eval-test
  (testing "Comb templates evaluted with clojure.eval"
    (is (= (cmb/eval (t/parse-template {:engine :comb} "foo") {}) "foo"))
    (is (= (cmb/eval (t/parse-template {:engine :comb} "<%= 10 %>") {}) "10"))
    (is (= (cmb/eval (t/parse-template {:engine :comb} "<%= x %>") {:x "foo"}) "foo"))
    (is (= (cmb/eval (t/parse-template {:engine :comb} "<%=x%>") {:x "foo"}) "foo"))
    (is (= (cmb/eval (t/parse-template {:engine :comb} "<% (doseq [x xs] %>foo<%= x %> <% ) %>") {:xs [1 2 3]})
           "foo1 foo2 foo3 "))))

(declare x)
(deftest fn-test
  (is (= ((cmb/fn [x] (t/parse-template {:engine :comb} "foo<%= x %>")) "bar")
         "foobar")))
